...
public class Player : BaseUserTrackModel
{
        ...
        string team, string league, string country, Guid suggestedByUserId)
    {
        if (string.IsNullOrWhiteSpace(name))
            return ResultDomain<Player>.Error(new MessageItem
            {
        ...
        if (age < 13 || age > 50)
            return ResultDomain<Player>.Error(new MessageItem
            {
                Code = ErrorCodes.COMMON_MESSAGE_INVALID_VALUE,
                Property = nameof(age),
                Table = nameof(Player)
            });

        var id = Guid.NewGuid();
        var slug = GenerateSlug(name, id);
    ...
    public ResultDomain Approve()
    {
        if (Status == PlayerStatus.Approved)
            ...
            {
                Code = ErrorCodes.PLAYER_ALREADY_APPROVED,
