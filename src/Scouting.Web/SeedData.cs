...
public static class SeedData
{
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        var admin   = MakeUser("admin@scouting.com",  "admin",       hash, admin: true);
        var emre    = MakeUser("emre@scout.com",      "emrekoç",     hash);
        var mehmet  = MakeUser("mehmet@scout.com",    "mehmetöz",    hash);
        var ahmet   = MakeUser("ahmet@scout.com",     "ahmetyılmaz", hash);
        var zeynep  = MakeUser("zeynep@scout.com",    "zeynepkaya",  hash);
        ...
        var semih = MakePlayer("Semih Kılıçsoy",      19, PlayerPosition.ST,
            ...
            imageUrl: "https://img.a.transfermarkt.technology/portrait/header/1024252-1694598397.jpg",
            marketValue: 12m);
        ...
        var berat = MakePlayer("Berat Özdemir",       22, PlayerPosition.CM,
            "Galatasaray", "Süper Lig", "Türkiye", zeynep.Id,
            marketValue: 6m);

        var yunus = MakePlayer("Yunus Akgün",         22, PlayerPosition.RW,
            "Galatasaray", "Süper Lig", "Türkiye", emre.Id,
            marketValue: 10m);

        db.Players.AddRange(arda, kenan, ferdi, semih, karetsas, berat, yunus);
        ...
        // Berat Özdemir analizi
        var a7 = MakeAnalysis(berat.Id, zeynep.Id,
            videoUrl: "https://www.youtube.com/watch?v=dQw4w9WgXcQ",
            general: "Berat Özdemir Galatasaray'ın orta saha yapısında önemli bir rol üstlenmektedir. Savunma organizasyonuna katkısı ve topu kazanma kapasitesiyle kendini farklılaştırmaktadır. Süper Lig standartlarında güvenilir bir oyuncu profili çizse de Avrupa futboluna adım atmak için bazı alanlarda gelişim gerekli.",
            technical: "Kısa pas oyunu güçlü. Uzun pas isabeti ortalama. Top kazanma ve savunma geçişlerinde etkin.",
            strengths: "Savunma katkısı, top kazanma, takım içi konumlanma.",
            weaknesses: "Hız değerleri üst düzey rekabet için yetersiz kalabilir. Hücuma katkı sınırlı.",
            score: 6.5m, summary: "Süper Lig'de güvenilir, Avrupa standartlarında gelişim gerektiren dengeli orta saha.");
        a7.Approve();
        for (var i = 0; i < 3; i++) a7.IncrementLikeCount();

        // Yunus Akgün analizi
        var a8 = MakeAnalysis(yunus.Id, emre.Id,
            videoUrl: "https://www.youtube.com/watch?v=dQw4w9WgXcQ",
            general: "Yunus Akgün Galatasaray'ın sağ kanadında düzenli performans sergilemektedir. Süratli ve direk oyun tarzıyla rakip sol bekler için ciddi bir tehdit unsuru oluşturmaktadır. Milli takım kadrolarında da yer bulmuş olması oyuncunun geldiği seviyeyi göstermektedir.",
            technical: "Sürat bazlı dribling oldukça etkili. Kros kalitesi gelişmiş. Ceza sahasına girişte soğukkanlılık artmış.",
            strengths: "Sürat, direk oyun tarzı, bir-bir geçişler.",
            weaknesses: "Savunmaya dönüş mesafesi bazen uzuyor. Bitiriş kalitesi tutarsız.",
            score: 6.8m, summary: "Galatasaray sağ kanadında sürat temelli tehdit kaynağı.");
        a8.Approve();
        for (var i = 0; i < 4; i++) a8.IncrementLikeCount();
        ...
        db.Analyses.AddRange(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10);
        ...
        db.Votes.AddRange(
            ...
            Vote.Create(berat.Id, zeynep.Id, VoteType.Up),
            Vote.Create(yunus.Id, mehmet.Id, VoteType.Down)
        );
